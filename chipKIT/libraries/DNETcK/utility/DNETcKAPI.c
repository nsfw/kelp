...
static void StopBoard(void)
{	
#if defined(_BOARD_MEGA_) || defined(_BOARD_CEREBOT_32MX7_)    
	LATEbits.LATE9 = 0; 	// low, disable the phy
#elif defined(_BOARD_CEREBOT_MX7CK_)
	LATAbits.LATA6 = 0; 	// low, disable the phy
#else
	#error	Board/CPU combination not defined
#endif
}
...
/*****************************************************************************
  Function:
	bool EthernetIsInitialzied(unsigned long msBlockMax, STATUS * pStatus)
...
bool EthernetIsInitialzied(unsigned long msBlockMax, STATUS * pStatus)
{
    ...
    else
    {
	    ...
	    while(AppConfig.Flags.bIsDHCPEnabled && !DHCPIsBound(0))
	    {
...
bool EthernetIsDNSResolved(const char * szHostName, byte * pIP, unsigned long msBlockMax, STATUS * pStatus)
{
    ...
    if(!EthernetIsInitialzied(msBlockMax, pStatus))
    {
        return(FALSE);
    }
...
void EthernetPeriodicTasks(void)
{
    ...
    if(fInPeriodicTasks  || fIsEthernetEngineStopped)
    {
        return;
    }
