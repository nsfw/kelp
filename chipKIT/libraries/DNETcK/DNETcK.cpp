...
**      bool DNETcK::begin(const MAC& mac, const IPv4& ip, const IPv4& ipGateway, const IPv4& subnetMask)
**      bool DNETcK::begin(const MAC& mac, const IPv4& ip, const IPv4& ipGateway, const IPv4& subnetMask, const IPv4& ipDns1)
**      bool DNETcK::begin(const MAC& mac, const IPv4& ip, const IPv4& ipGateway, const IPv4& subnetMask, const IPv4& ipDns1, const IPv4& ipDns2)
**
**	Synopsis:   
**      This initializes the Ethernet hardware and the IP Stack
**      For both UDP and TCP. This will return immediately after 
**      hardware initialization and IsInitialzied should be called
**      if it is important to know that the stack is up and running.
**      usually it is not needed to explicity call IsInitialzied as most
...
bool DNETcK::begin(const IPv4& ip)
{
    return(begin(zMAC, ip));
}
bool DNETcK::begin(const IPv4& ip, const IPv4& ipGateway)
{
    return(begin(zMAC, ip, ipGateway));
}
bool DNETcK::begin(const IPv4& ip, const IPv4& ipGateway, const IPv4& subnetMask)
{
    return(begin(zMAC, ip, ipGateway, subnetMask));
}
bool DNETcK::begin(const IPv4& ip, const IPv4& ipGateway, const IPv4& subnetMask, const IPv4& ipDns1)
{
    return(begin(zMAC, ip, ipGateway, subnetMask, ipDns1));
}
bool DNETcK::begin(const IPv4& ip, const IPv4& ipGateway, const IPv4& subnetMask, const IPv4& ipDns1, const IPv4& ipDns2)
{
    return(begin(zMAC, ip, ipGateway, subnetMask, ipDns1, ipDns2));
}
bool DNETcK::begin(const MAC& mac)
{
    return(begin(mac, zIPv4));
}
bool DNETcK::begin(const MAC& mac, const IPv4& ip)
{
    IPv4 ipGateway = ip;
    ipGateway.rgbIP[2] = 0;
    ipGateway.rgbIP[3] = 1;
    return(begin(mac, ip, ipGateway));
}
bool DNETcK::begin(const MAC& mac, const IPv4& ip, const IPv4& ipGateway)
{
    return(begin(mac, ip, ipGateway, (IPv4) {255,255,255,0}));
}
bool DNETcK::begin(const MAC& mac, const IPv4& ip, const IPv4& ipGateway, const IPv4& subnetMask)
{
    return(begin(mac, ip, ipGateway, subnetMask, ipGateway));
}
bool DNETcK::begin(const MAC& mac, const IPv4& ip, const IPv4& ipGateway, const IPv4& subnetMask, const IPv4& ipDns1)
{
    return(begin(mac, ip, ipGateway, subnetMask, ipDns1, zIPv4));
}
bool DNETcK::begin(const MAC& mac, const IPv4& ip, const IPv4& ipGateway, const IPv4& subnetMask, const IPv4& ipDns1, const IPv4& ipDns2)
{
    if(_fBegun)
    ...
    _ipDns1 = ipDns1;
    _ipDns2 = ipDns2;
    EthernetBegin(mac.rgbMAC, ip.rgbIP, ipGateway.rgbIP, _subMask.rgbIP, _ipDns1.rgbIP, _ipDns2.rgbIP);
    return(true);
}
...
/***	bool DNETcK::isInitialzied(void)
**      bool DNETcK::isInitialzied(DNETcK::STATUS * pStatus)
**      bool DNETcK::isInitialzied(unsigned long msBlockMax)
**      bool DNETcK::isInitialzied(unsigned long msBlockMax, DNETcK::STATUS * pStatus)
**
**	Synopsis:   
...
**      msBlockMax  Will cause IsInitialzied to block waiting for either successful initalization or 
...
bool DNETcK::isInitialzied(void)
{
    return(isInitialzied(_msDefaultTimeout, NULL));
}
bool DNETcK::isInitialzied(DNETcK::STATUS * pStatus)
{
    return(isInitialzied(_msDefaultTimeout, pStatus));
}
bool DNETcK::isInitialzied(unsigned long msBlockMax)
{
    return(isInitialzied(msBlockMax, NULL));
}
bool DNETcK::isInitialzied(unsigned long msBlockMax, DNETcK::STATUS * pStatus)
{
    if(!_fIsInitialized)
    {
     _fIsInitialized = EthernetIsInitialzied(msBlockMax, pStatus);
...
bool DNETcK::isDNSResolved(const char * szHostName, IPv4 * pIP, unsigned long msBlockMax, DNETcK::STATUS * pStatus)
{
    if(!isInitialzied(msBlockMax))
    {
        if(pStatus != NULL)
        {
            *pStatus = DNETcK::NetworkNotInitialized;
        }
        return(false);
    }
