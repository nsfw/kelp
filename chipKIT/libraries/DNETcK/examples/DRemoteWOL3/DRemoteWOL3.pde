...
#define cHistoryEntries 1024

static int cSecTimeout = 10;
...
namespace SLED {
    ...
    void setLED(SLED::STATE state)
    {
        ...
        digitalWrite(PIN_LED1, blinkState);
    }
}
...
CMDMSG state = DYNAMICIPBEGIN;
CMDMSG stateNext = CHECKSTATICIP;
...
void setup() 
{
    ...
    Serial.begin(9600);  
    Serial.println("Start of DRemoteWOL 3.0");
}
...
void sendBroadcastHistory(void)
{
    ...
    if(cSend1 > 0)
    {
        tcpClient.writeStream((byte *) &bhs.rgBHE[iStart], cSend1 * sizeof(BHE)); 
    }

    if(cSend2 > 0)
    {
        tcpClient.writeStream((byte *) &bhs.rgBHE[0], cSend2 * sizeof(BHE)); 
    }
}
